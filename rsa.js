...
// RSA PUBLIC ENCRYPT -> PRIVATE DECRYPT //
 myMSG = 여기에 AES의 대칭키 입력되어야 함.
  
 
function encrypt(originMSG){
 encmsg = crypto.publicEncrypt(PUBLIC_KEY, Buffer.from(originMSG, 'utf8') ).toString('base64');
 console.log("\nEncrypted with public key : "+encmsg);
 
}

function decrypt(encmsg){
msg = crypto.privateDecrypt(PRIVATE_KEY, Buffer.from(encmsg, 'base64'));
